...
public class PacmanMove : MonoBehaviour {
	...
	private void FixedUpdate()
	{
		...
		{
			if (((direction==1) || Input.GetKey(KeyCode.W)) && Valid(Vector2.up))
			{
				dest = (Vector2)transform.position + Vector2.up;
			}
			if (((direction==2) || Input.GetKey(KeyCode.S)) && Valid(Vector2.down))
			{
				dest = (Vector2)transform.position + Vector2.down;
			}
			if (((direction==3) || Input.GetKey(KeyCode.A)) && Valid(Vector2.left))
			{
				dest = (Vector2)transform.position + Vector2.left;
			}
			if (((direction==4) || Input.GetKey(KeyCode.D)) && Valid(Vector2.right))
			{
